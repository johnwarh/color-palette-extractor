...
const main = () => { //called when load button clicked
	...
	console.log(' filename = ' + imgFile.files[0]);
	clean_canvas_and_palette();
	fileName.innerHTML = file.name;
	const n_colors = 2**colors.value;
	...
	reader.onload = () => {
		image.onload = () => {
			...
			// setup canvas for quantized color image
	...
	const buildPalette = (colorsList) => { // colorsList = quantColors
		let hslColors = '',
			hslColorsComp = '';
	...
	const rgbToHex = (pixel) => {
		//console.log('pixel = ' + JSONstringify(pixel));
